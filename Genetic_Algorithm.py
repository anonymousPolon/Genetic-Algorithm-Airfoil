    ...
    try:
        ...
        try:
            ...
            if polar_data.size == 0:
                print(
                    f"Polar file {polar_file_path} has no data after skipping 12 rows."
                )
                return False
            else:
                return True
...
def fitness_function(ga_instance, Au_Al, solution_idx):
    ...
    plt.axis("equal")
    plt.savefig(
        f"figure/Generated Airfoil - Generation {generation} Solution {solution_idx}"
    )
    plt.close()
    ...
    if has_self_intersection(airfoil_coordinates):
        print(
            f"Airfoil in generation {generation}, solution {solution_idx} has self-intersections. Penalizing fitness."
        )
...
def main():
    ...
    best_generation = np.argmax(
        best_fitnesses
    )  # Index of the generation with the best fitness

    print("Best Airfoil Parameters:", best_solution)
    print("Best Fitness Value:", best_fitness)
    print(
        f"Best Airfoil is: generated_airfoil_gen{best_generation+1}_sol{best_solution_idx}.dat"
    )
